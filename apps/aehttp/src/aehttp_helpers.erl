...
get_block(Fun, Req, Encoding) ->
    get_block(Fun, Req, Encoding, true).

get_block(Fun, Req, Encoding, AddHash) when is_function(Fun, 0) ->
    case get_block_from_chain(Fun) of
        {ok, Block} ->
            case read_tx_encoding_param(Req, Encoding) of
                {error, Err} ->
                    Err;
                {ok, TxEncoding} ->
                    DataSchema =
